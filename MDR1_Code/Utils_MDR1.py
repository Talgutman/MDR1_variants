...
''' 
A dictionary with general information on our three variants of interest
'''
variant_info = {
    1: {
        "variant_name": "T1236C",
...
''' a color pallete that we will often use through out the paper '''
colors = sns.color_palette("Set3", 10)

''' 
Cancer types clustered as suggested in Gao et. al (Clustering cancers by shared transcriptional risk reveals novel targets for can-cer therapy) 
'''
cancertype_clusters = {
    "Metabolic Cancers" : ["TCGA-LAML", "TCGA-UCS", "TCGA-HNSC", "TCGA-ESCA", "TCGA-UVM", "TCGA-CHOL", "TCGA-LIHC"],
    "Proliferative Cancers": ["TCGA-BLCA", "TCGA-SKCM", "TCGA-SARC", "TCGA-COAD", "TCGA-UCEC", "TCGA-MESO", "TCGA-ACC", "TCGA-LUAD", "TCGA-KIRC", "TCGA-KIRP", "TCGA-DLBC", "TCGA-TGCT", "TCGA-PRAD"],
    "Inflammatory Cancers" : ["TCGA-PAAD", "TCGA-LGG", "TCGA-CESC", "TCGA-GBM", "TCGA-READ", "TCGA-LUSC", "TCGA-BRCA", "TCGA-STAD", "TCGA-THCA", "TCGA-OV", "TCGA-KICH", "TCGA-PCPG", "TCGA-THYM"]
}


''' 
A dictionary where the keys as amino-acids and the values are the codons that encode them
'''
SynonymousCodons = {
  "C": ["TGT", "TGC"],
  "D": ["GAT", "GAC"],
...
''' 
A dictionary where the values are the amino acids and the keys are the codons that encode them
'''
reverse_dict = {value: key for key in SynonymousCodons for value in SynonymousCodons[key]}
 
''' A dictionary that maps between the DNA alphabet and it's numeric representation '''
...
def mutate_cds_sequence(sequence: str, position: int, change_to: str) -> str:
    '''
    Perform a single mutation in a sequence. "position" is the 1 based position where you want to insert the mutation. 
    "change_to" is the nucleotide you wish that the new sequence will hold. 
    '''
...
def nansumwrapper(vectors_to_sum: np.array, axis: int) -> np.array:
    '''
...
def get_cancer_muts_cur_gene(gene_name):
    ...
    '''
    cancerous_muts_path = f"../Data/TCGA/AllGenes/{gene_name}/"
...
def get_muts_single_patient(case_id: str, patient_cancer_type: str) -> pd.DataFrame:
    '''
    ...
    return(cur_patient_muts)
 

def reverse_complement(s: str, complement: dict = None) -> str: 
    """
    Credits to Yoram Zarai who wrote this function
